...
if __name__ == '__main__':
	...
	# required args
	parser.add_argument("model", help="Path to a JSON model file (i.e. from CNNPolicy.save_model())", required=True)
	...
	parser.add_argument("--minibatch", "-B", help="Size of training data minibatches. Default: 16", default=16)
	parser.add_argument("--epochs", "-E", help="Total number of iterations on the data. Default: 10", default=10)
	parser.add_argument("--learning-rate", "-r", help="Learning rate - how quickly the model learns at first. Default: .03", default=.03)
	parser.add_argument("--decay", "-d", help="The rate at which learning decreases. Default: .0001", default=.0001)
	parser.add_argument("--workers", "-w", help="Number of 'workers' workon on batch generator in parallel. Default: 4", default=4)
	...
	parser.add_argument("--train-dev-test", help="Fraction of data to use for training/dev/test. Must sum to 1. Invalid if restarting training",
		nargs=3, default=[0.93, .05, .02])
	args = parser.parse_args()

	# TODO - add arg for verbose mode
	...
	n_train_data = np.floor(args.train_dev_test[0] * n_total_data)
	n_dev_data = np.floor(args.train_dev_test[1] * n_total_data)
	n_test_data = n_total_data - (n_train_data + n_dev_data)
	...
	# training indices are the first consecutive set of shuffled indices, dev next, then test gets the remainder
	train_indices = shuffle_indices[0:n_train_data]
	dev_indices = shuffle_indices[n_train_data:n_train_data+n_dev_data]
	test_indices = shuffle_indices[n_train_data+n_dev_data:]
	...
	train_data_generator = shuffled_hdf5_batch_generator(
		...
		args.minibatch,
		BOARD_TRANSFORMATIONS)
	dev_data_generator = shuffled_hdf5_batch_generator(
		dataset["states"],
		dataset["actions"],
		dev_indices,
		args.minibatch,
